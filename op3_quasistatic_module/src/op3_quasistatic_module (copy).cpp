...
void QuasistaticControlModule::setJointCallback(const sensor_msgs::JointState::ConstPtr &msg)
{
  ...
  // generate trajectory
  tra_gene_thread_ = new boost::thread(boost::bind(&QuasistaticControlModule::jointTraGeneThread, this));
 ...
 */
Eigen::MatrixXd QuasistaticControlModule::calcMinimumJerkTraPVA(double pos_start, double vel_start, double accel_start,
...
bool QuasistaticControlModule::checkSelfCollision()
{
...
bool QuasistaticControlModule::getDiff(OP3KinematicsDynamics *kinematics, int end_index, int base_index, double &diff)
{
